...
import {
  ...
  type EnqueueThreadMessageInput,
  type UpdateThreadQueuedMessageInput,
...
export function createThreadEnvironmentAtoms<R, E>(
  ...
  const commands = {
    ...
    enqueueMessage: createEnvironmentCommand(runtime, {
      label: "environment-data:commands:thread:enqueue-message",
      execute: (input: EnqueueThreadMessageInput) => enqueueThreadMessage(input),
      scheduler,
      concurrency,
    }),
    updateQueuedMessage: createEnvironmentCommand(runtime, {
      label: "environment-data:commands:thread:update-queued-message",
      execute: (input: UpdateThreadQueuedMessageInput) => updateThreadQueuedMessage(input),
      scheduler,
      concurrency,
    }),
